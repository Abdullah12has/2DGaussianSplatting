...
class GaussianModel:
    ...
    def reinitialize_from_depth(self, pts, rgb, training_args):
        ...
        Preserves exposure optimization state if active.
        ...
        # Preserve exposure state before reinit
        saved_exposure = self._exposure
        saved_exposure_mapping = self._exposure_mapping
        ...
        # Restore exposure state so it survives reinitialization
        self._exposure = saved_exposure
        self._exposure_mapping = saved_exposure_mapping
    ...
    def replace_tensors_to_optimizer(self, inds=None):
        """Replace tensors in optimizer, optionally resetting state for specific indices."""
        tensors_dict = {
            "xyz": self._xyz,
    ...
    def _update_params(self, idxs, ratio):
        """Compute new opacity and scale for relocated/added Gaussians."""
        ...
        new_scaling = self.scaling_inverse_activation(new_scaling.reshape(-1, 2))  # 2D for surfels
    ...
    def _sample_alives(self, probs, num, alive_indices=None):
        """Sample indices weighted by opacity probability."""
    ...
    def relocate_gs(self, dead_mask=None):
        """Relocate dead (low-opacity) Gaussians to live ones via MCMC sampling."""
        if dead_mask.sum() == 0:
            return
    ...
    def add_new_gs(self, cap_max):
        """Add new Gaussians by sampling from high-opacity regions (5% growth per step)."""
